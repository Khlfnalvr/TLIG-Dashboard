...
/// a syntactically valid address whose domain is <c>its.ac.id</c> or any
/// sub-domain of it (e.g. <c>mhs.its.ac.id</c>, <c>student.its.ac.id</c>).
///
...
public static class EmailPolicy
{
    ...
    /// <summary>True when the address domain is its.ac.id or a sub-domain of it.</summary>
    public static bool IsAllowedDomain(string? email)
    {
        ...
        var domain = e[(at + 1)..];
        return domain == RootDomain || domain.EndsWith("." + RootDomain, StringComparison.Ordinal);
    }
