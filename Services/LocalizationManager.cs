...
public sealed class LocalizationManager : INotifyPropertyChanged
{
    ...
    public string HeQ_TurnArrivedTitle => T(nameof(HeQ_TurnArrivedTitle));
    public string HeQ_TurnArrivedMsg   => T(nameof(HeQ_TurnArrivedMsg));
    public string HeQ_HoldEndedTitle   => T(nameof(HeQ_HoldEndedTitle));
    public string HeQ_HoldEndedMsg     => T(nameof(HeQ_HoldEndedMsg));
    public string HeQ_TimeLeftTitle    => T(nameof(HeQ_TimeLeftTitle));
    public string HeQ_TimeLeftMsg      => T(nameof(HeQ_TimeLeftMsg));     // {0} sisa menit, {1} batas menit
    public string HeQ_TimeLeftLabel    => T(nameof(HeQ_TimeLeftLabel));   // {0} sisa waktu mm:ss
    public string HeQ_NoTimeLimit      => T(nameof(HeQ_NoTimeLimit));
    public string HeQ_ForceDoneTitle   => T(nameof(HeQ_ForceDoneTitle));
    public string HeQ_ForceDoneMsg     => T(nameof(HeQ_ForceDoneMsg));    // {0} nama yang dicabut
    public string HeQ_ForceRigWarnMsg  => T(nameof(HeQ_ForceRigWarnMsg)); // {0} nama, {1} alasan
    public string HeQ_ForceFailedMsg   => T(nameof(HeQ_ForceFailedMsg));
    public string HeQ_ExitBlockedTitle => T(nameof(HeQ_ExitBlockedTitle));
    public string HeQ_ExitBlockedMsg   => T(nameof(HeQ_ExitBlockedMsg));  // {0} alasan
    public string HeQ_RigNoBridge      => T(nameof(HeQ_RigNoBridge));
    public string HeQ_RigWriteFailed   => T(nameof(HeQ_RigWriteFailed));
    public string HeQ_TroubleHoldKept  => T(nameof(HeQ_TroubleHoldKept));  // {0} nama, {1} alasan
    public string HeQ_TroubleExpired   => T(nameof(HeQ_TroubleExpired));   // {0} nama, {1} alasan
    public string HeQ_TroubleForced    => T(nameof(HeQ_TroubleForced));    // {0} alasan
    ...
    public string HeH_StatusInterrupted => T(nameof(HeH_StatusInterrupted));
    public string HeMy_Title            => T(nameof(HeMy_Title));
    public string HeMy_Subtitle         => T(nameof(HeMy_Subtitle));
    public string HeMy_Empty            => T(nameof(HeMy_Empty));
    public string HeMy_Export           => T(nameof(HeMy_Export));
    ...
    public string Um_QueueHolding    => T(nameof(Um_QueueHolding));   // {0} lama pegang
    public string Um_QueueWaiting    => T(nameof(Um_QueueWaiting));   // {0} posisi antrean
    public string Um_QueueIdle       => T(nameof(Um_QueueIdle));
    ...
    private static readonly Dictionary<string, Dictionary<string, string>> _strings = new()
    {
        ...
        ["en"] = new()
        {
            ...
            [nameof(Um_QueueHolding)] = "Running the plant · {0}",
            [nameof(Um_QueueWaiting)] = "Waiting · #{0}",
            [nameof(Um_QueueIdle)]  = "—",
            ...
            [nameof(HeQ_QueuedMsg)]    = "The plant is in use by {0} ({1}), for {2} now. You are #{3} of {4} in the queue. Nothing was sent to the plant — you will be notified when your turn starts.",
            ...
            [nameof(HeQ_TurnArrivedTitle)] = "Your turn has started",
            [nameof(HeQ_TurnArrivedMsg)]   = "The plant is yours now — press RUN on the Dashboard to start your experiment.",
            [nameof(HeQ_HoldEndedTitle)]   = "Your turn has ended",
            [nameof(HeQ_HoldEndedMsg)]     = "The plant was stopped and handed to the next person in the queue. Press RUN again to rejoin the queue.",
            [nameof(HeQ_TimeLeftTitle)]    = "Time running out",
            [nameof(HeQ_TimeLeftMsg)]      = "{0} minute(s) left of your {1}-minute turn. When it runs out the plant is stopped automatically and the turn moves to the next person — your data is saved either way.",
            [nameof(HeQ_TimeLeftLabel)]    = "{0} left",
            [nameof(HeQ_NoTimeLimit)]      = "no time limit",
            [nameof(HeQ_ForceDoneTitle)]   = "Control released",
            [nameof(HeQ_ForceDoneMsg)]     = "The plant was stopped and {0}'s turn was released.",
            [nameof(HeQ_ForceRigWarnMsg)]  = "{0}'s turn was released, but the plant could NOT be stopped from here ({1}). Check the rig and LabVIEW directly before letting anyone run.",
            [nameof(HeQ_ForceFailedMsg)]   = "Could not release the turn — the server did not accept the request.",
            [nameof(HeQ_ExitBlockedTitle)] = "Turn kept — plant not stopped",
            [nameof(HeQ_ExitBlockedMsg)]   = "Your turn was NOT released because the plant could not be stopped ({0}). It is still yours; ask a lecturer or assistant to force-release it after checking the rig.",
            [nameof(HeQ_RigNoBridge)]      = "the Python bridge is not running, so STOP cannot reach LabVIEW",
            [nameof(HeQ_RigWriteFailed)]   = "the STOP command could not be written to pid_bridge.json",
            [nameof(HeQ_TroubleHoldKept)]  = "{0}'s turn was NOT released because the plant could not be stopped ({1}). Check the rig, then force-release from the queue panel.",
            [nameof(HeQ_TroubleExpired)]   = "{0} is over the time limit but the turn was NOT released because the plant could not be stopped ({1}). Check the rig, then force-release from the queue panel.",
            [nameof(HeQ_TroubleForced)]    = "The force release went through, but the plant may still be running ({0}). Check the rig and LabVIEW directly.",
            ...
            [nameof(HeH_StatusInterrupted)] = "Interrupted",
            [nameof(HeMy_Title)]            = "MY EXPERIMENT HISTORY",
            [nameof(HeMy_Subtitle)]         = "Every plant run you have made, newest first — your own runs only.",
            [nameof(HeMy_Empty)]            = "You have not run the plant yet. Press RUN on the Dashboard to start one.",
            [nameof(HeMy_Export)]           = "Export CSV",
        ...
        ["id"] = new()
        {
            ...
            [nameof(Um_QueueHolding)] = "Sedang memakai plant · {0}",
            [nameof(Um_QueueWaiting)] = "Mengantre · ke-{0}",
            [nameof(Um_QueueIdle)]  = "—",
            ...
            [nameof(HeQ_QueuedMsg)]    = "Plant sedang dipakai {0} ({1}), sudah {2}. Anda antrean ke-{3} dari {4}. Tidak ada perintah yang dikirim ke plant — Anda akan diberi tahu begitu giliran Anda tiba.",
            ...
            [nameof(HeQ_TurnArrivedTitle)] = "Giliran Anda sudah tiba",
            [nameof(HeQ_TurnArrivedMsg)]   = "Plant sekarang milik Anda — tekan JALANKAN di Dashboard untuk memulai percobaan.",
            [nameof(HeQ_HoldEndedTitle)]   = "Giliran Anda berakhir",
            [nameof(HeQ_HoldEndedMsg)]     = "Plant sudah dihentikan dan gilirannya berpindah ke antrean berikutnya. Tekan JALANKAN lagi untuk masuk antrian.",
            [nameof(HeQ_TimeLeftTitle)]    = "Waktu giliran hampir habis",
            [nameof(HeQ_TimeLeftMsg)]      = "Sisa {0} menit dari jatah {1} menit Anda. Kalau habis, plant dihentikan otomatis dan giliran berpindah ke orang berikutnya — data percobaan Anda tetap tersimpan.",
            [nameof(HeQ_TimeLeftLabel)]    = "sisa {0}",
            [nameof(HeQ_NoTimeLimit)]      = "tanpa batas waktu",
            [nameof(HeQ_ForceDoneTitle)]   = "Kendali dilepas",
            [nameof(HeQ_ForceDoneMsg)]     = "Plant sudah dihentikan dan giliran {0} dilepas.",
            [nameof(HeQ_ForceRigWarnMsg)]  = "Giliran {0} sudah dilepas, tetapi plant TIDAK bisa dihentikan dari sini ({1}). Periksa rig dan LabVIEW langsung sebelum ada yang menjalankan lagi.",
            [nameof(HeQ_ForceFailedMsg)]   = "Giliran tidak bisa dilepas — server menolak permintaannya.",
            [nameof(HeQ_ExitBlockedTitle)] = "Giliran ditahan — plant belum berhenti",
            [nameof(HeQ_ExitBlockedMsg)]   = "Giliran Anda TIDAK dilepas karena plant tidak bisa dihentikan ({0}). Giliran itu masih milik Anda; minta Dosen atau Asisten mencabutnya paksa setelah rig diperiksa.",
            [nameof(HeQ_RigNoBridge)]      = "jembatan Python tidak berjalan, jadi perintah BERHENTI tidak sampai ke LabVIEW",
            [nameof(HeQ_RigWriteFailed)]   = "perintah BERHENTI gagal ditulis ke pid_bridge.json",
            [nameof(HeQ_TroubleHoldKept)]  = "Giliran {0} TIDAK dilepas karena plant tidak bisa dihentikan ({1}). Periksa rig, lalu cabut paksa dari panel antrian.",
            [nameof(HeQ_TroubleExpired)]   = "{0} sudah melewati batas waktu tetapi gilirannya TIDAK dilepas karena plant tidak bisa dihentikan ({1}). Periksa rig, lalu cabut paksa dari panel antrian.",
            [nameof(HeQ_TroubleForced)]    = "Cabut paksa tetap diteruskan, tetapi plant mungkin masih menyala ({0}). Periksa rig dan LabVIEW langsung.",
            ...
            [nameof(HeH_StatusInterrupted)] = "Terputus",
            [nameof(HeMy_Title)]            = "RIWAYAT PERCOBAAN SAYA",
            [nameof(HeMy_Subtitle)]         = "Semua percobaan plant yang pernah Anda jalankan, terbaru di atas — hanya milik Anda sendiri.",
            [nameof(HeMy_Empty)]            = "Belum ada percobaan. Tekan JALANKAN di Dashboard untuk memulai.",
            [nameof(HeMy_Export)]           = "Ekspor CSV",
